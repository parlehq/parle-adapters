...
- Carry the shared card rendering of client 0.8.55 and the `idle_wake_suspended` status reason of MCP server 0.7.64 into the Command Code mod artifact; the mod runs no hook-bridge idle wake, so no Command Code runtime behavior changed (parlehq/parle-adapters#185).
