...
test("skill configurator uses native MCP registration and preserves unrelated hooks", async () => {
  ...
  try {
    ...
    assert.match(commands[1], /^mcp add --transport stdio --scope user --env PARLE_RESPONSIVE_DELIVERY=hook-bridge --env PARLE_INTEGRATION_NAME=@parlehq\/command-code-adapter --env PARLE_INTEGRATION_VERSION=0\.3\.0 parle -- node .*skills\/parle\/server\/parle-mcp\.js\s*$/);
