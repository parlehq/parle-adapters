// src/index.ts
import { existsSync as existsSync9, lstatSync as lstatSync7, readFileSync as readFileSync6, statSync as statSync3 } from "node:fs";
...
var DEFAULT_VERSION = "2026-09-08";
...
async function claimAliasWithRecovery(transport, candidate, alias, expectedGeneration, signal, expectedAliasIdentityId) {
  ...
  for (let attempt = 1; attempt <= CLAIM_RECOVERY_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (error instanceof ParleApiError && ["invalid_response", "alias_claim_identity_changed"].includes(error.code || ""))
        throw error;
...
function deleteProfile(params, options) {
  ...
  try {
    ...
    if (error instanceof ProfileDeletionError)
      throw error;
    if (error instanceof SafeFileError && error.code === "lock_contended") {
...
import { basename as basename2, dirname as dirname6, isAbsolute as isAbsolute2, join as join8, parse, relative, resolve, sep } from "node:path";
...
function ceremonyPath(config) {
  return join7(config.stateDir, "hardening", CEREMONY_DIR);
}
function rootPath(config) {
  return join7(config.stateDir, "hardening");
}
function outputPath(config, file) {
  return join7(ceremonyPath(config), file);
}
...
var ParleHardeningClient = class {
  ...
  fingerprint(config) {
    return createHash2("sha256").update(config.sessionCookie, "utf8").digest("hex");
  }
  ensureRoot(config) {
  ...
  async acknowledgeRecoveryStored() {
    ...
    const path = join7(ceremonyPath(config), ACK_FILE);
  ...
  async status(config) {
    ...
    if (whoami.assurance === "hardened") {
      if (state.phase === "hardened_recovery_captured" && state.recoveryCaptured && state.assuranceVerified && existsSync5(outputPath(config, "recovery-codes.txt"))) {
        try {
          assertSecureFile(outputPath(config, "recovery-codes.txt"), "protected recovery codes");
  ...
  async finalize(config) {
    ...
    const ack = join7(ceremonyPath(config), ACK_FILE);
...
function inviteDirectory(config, create) {
  const directory = join8(config.stateDir, "invites");
  if (create) {
...
function resolveAccountConfig(cwd, env) {
  ...
  if (!config.sessionCookie)
    throw new Error(`Parle human session is not configured. Run parle_login complete or mint-from-session so ${join8(dirname6(config.catalogPath), "session")} exists.`);
  return config;
}
...
var ParleAccountClient = class {
  ...
  async connectOwnAgent(params, signal) {
    ...
    if (params.action === "preview" && selected) {
      ...
      const profiles2 = existsSync6(config.catalogPath) ? parseProfiles(readFileSync3(config.catalogPath, "utf8"), config.catalogPath) : /* @__PURE__ */ new Map();
...
var ParleAgentClient = class _ParleAgentClient {
  ...
  recordRoomOperationTerminalCause(error, roomId, requestLocal = false) {
    const api = error instanceof ParleApiError ? error : void 0;
  ...
  async claimAliasWithRecovery(candidate, alias, expectedGeneration, signal, aliasIdentityId) {
    ...
    try {
      const result = await claimAliasWithRecovery(this.aliasTransport(), candidate, alias, expectedGeneration, signal, aliasIdentityId);
  ...
  async withRebootstrap(fn, signal, terminalOwner = "automatic") {
    ...
    try {
      ...
      if (error instanceof ParleApiError && error.code === "alias_context_stale" && this.runtime.sessionAlias)
  ...
  async roomDetails(params = {}, signal) {
    ...
    return this.bootstrapGeneration !== generation2 && result && typeof result === "object" ? { ...result, roomId, session: this.sessionEstablishedBlock() } : result;
  }
  ...
  async send(params, signal) {
    ...
    try {
      const details = await this.withDataPlane(() => this.withRebootstrap(async () => {
        ...
        return { ...result, roomId, idempotencyKey, ...clientWarnings ? { clientWarnings } : {}, ...deliveryStatus ? { deliveryStatus } : {}, ...this.bootstrapGeneration !== generation2 ? { session: this.sessionEstablishedBlock() } : {} };
      }, signal, "request"));
  ...
  async submitReply(params, signal) {
    ...
    try {
      return await this.withDataPlane(() => this.withRebootstrap(async () => {
        ...
        const deliveryStatus = summarizeSendDelivery(result);
        return { ...result, roomId, idempotencyKey, ...deliveryStatus ? { deliveryStatus } : {}, ...this.bootstrapGeneration !== generation2 ? { session: this.sessionEstablishedBlock() } : {} };
      }, signal, "request"));
    } catch (error) {
      if (error instanceof ParleApiError) {
        this.recordRoomOperationTerminalCause(error, roomId);
...
function resolveConfig2(cwd, profileOverride = activeProfileOverride) {
  const projectEnv = readKeyValueFile2(join11(cwd, ".env"));
...
function sessionCookieFilePath2(catalogPath) {
  return join11(dirname8(catalogPath), "session");
}
...
function injectionFence() {
  ...
  return {
    ...
    roomId: view.roomId,
    sessionAlias: view.sessionAlias,
    ...view.aliasIdentityId && typeof view.sessionGeneration === "number" ? { aliasContext: { aliasIdentityId: view.aliasIdentityId, aliasGeneration: view.sessionGeneration } } : {},
    agentSessionId: view.agentSessionId,
    sourceId: view.agentSessionId
  };
}
...
function assertDeliveryFenceCurrent(fence) {
  if (!client?.responsiveDeliveryFenceCurrent?.(fence)) {
  ...
  if (client) {
    try {
      ...
      if (shutdownWarning) {
        ...
        try {
          ctx.ui?.notify?.(shutdownWarning, "warning");
        } catch {
        }
      }
