...
test("status publishes a display-safe runtime snapshot", async () => {
  ...
  assert.deepEqual(snapshot.adapter, { name: "@parlehq/pi-extension", version: "0.7.48" });
...
test("Pi JSON, generic agent request, and wake use one protected process identity", async () => {
  ...
  for (const call of calls) {
    ...
    assert.equal(call.headers["Parle-Client-Version"], "0.7.48");
...
test("generic parle_request honestly excludes human-session auth", async () => {
  ...
  assert.deepEqual(status.details.humanSession.supportedTools, ["parle_rooms", "parle_room_participants", "parle_room_capacity_recovery", "parle_login", "parle_create_room", "parle_create_own_agent", "parle_delete_own_agent", "parle_end_own_session", "parle_add_own_agent_seat", "parle_harden_account", "parle_mint_principal_invite", "parle_claim_principal_invite", "parle_accept_room_invitation", "parle_connect_own_agent"]);
});
