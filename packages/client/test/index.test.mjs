...
test("adapter owns one release-pinned DEFAULT_VERSION without a contract bundle", () => {
  ...
  assert.equal(DEFAULT_VERSION, "2026-08-08");
  assert.match(protocolSrc, /DEFAULT_VERSION = "2026-08-08"/);
...
test("PARLE_VERSION is adapter-owned unless explicitly set in process env", () => {
  ...
  try {
    ...
    assert.equal(defaultCfg.version.value, "2026-08-08");
    ...
    assert.equal(cleanCfg.version.value, "2026-08-08");
...
test("unsupported version errors include source, default, and server versions", async () => {
  const client = new ParleAgentClient({
    ...
    fetch: async () => json({ error: { code: "unsupported_parle_version", message: "unsupported Parle-Version header", supported: ["2026-08-08"], current: "2026-08-08" } }, 400),
  });
  await assert.rejects(
    () => client.requestJson("/v/test"),
    /Sent Parle-Version bad-version from env; adapter default is 2026-08-08\. Server supports 2026-08-08\. Unset the stale PARLE_VERSION override or upgrade the adapter\./,
  );
});
...
test("version error hint preserves supported-version precedence", () => {
  ...
  assert.equal(formatVersionErrorHint(cfg, { supported: ["new"], current: "also-new" }), " Sent Parle-Version old from env; adapter default is 2026-08-08. Server supports new. Unset the stale PARLE_VERSION override or upgrade the adapter.");
});
