...
The release is pinned to `Parle-Version: 2026-08-05`. Session creation always sends `{}`. When `PARLE_SESSION_ALIAS` is configured, the client creates an anonymous candidate, enters the configured room, verifies candidate wake readiness, reads the durable alias generation through the core alias lookup, and submits one exact generation-fenced claim. A failed claim is never replayed. Recovery prepares a fresh candidate and re-reads the durable fence, including after the prior owner expires.
