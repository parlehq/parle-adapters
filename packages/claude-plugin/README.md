...
See the [canonical adapter topology](../../docs/design/adapter-topology.md#claude-code) for the MCP child, hook bridge, Monitor attachment, Phase B boundary, and delivery stages. This README owns Claude installation and host-specific behavior.
...
Hooks run at `SessionStart` (bind plus known-address context), `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `Stop`. Hook calls containing Claude's supported `agent_id` subagent discriminator perform no delivery IPC, so a subagent cannot drain or acknowledge its parent's queue. `Stop` is required: a Monitor wake can produce a turn that calls no tool, and without a terminal boundary that queued delivery would strand until the next prompt. Because `.mcp.json` env is snapshotted at launch, adopting this release requires a Claude Code restart.

Idle wake is a loopback WebSocket (`ws://127.0.0.1:<port>/<token>`) the bridge serves because `.mcp.json` sets `PARLE_HOST_IDLE_WAKE=claude-monitor`. Claude attaches to it through its Monitor tool with `persistent: true`, which the harness tracks as a `monitor_ws` task. The address is a per-process random token handed only to the bound session's hook inside the owner-only `take` response; it never appears in `parle_status`, runtime snapshots, evidence, or logs, and it rotates when the hook binding moves to another session. The attachment opens no Parle session or network connection and owns no delivery state. Bridge `watching` proves only controller health. `waiterAttached` proves only that one Monitor peer is connected, not that a frame began a model turn. A healthy bound bridge without that peer reports `idle_wake_unarmed` instead of claiming idle wake is armed.

At an eligible `Stop`, the hook makes one bounded request through non-error hook context for Claude to call the Monitor tool once with that URL. When that Stop also drains delivery, the server-framed delivery comes first and the attachment instruction follows in the same continuation, because the global Stop fence prevents a second continuation. Injection is written and committed before Claude can act. When that fence is already active, Parle performs no Stop bridge IPC; queued rows remain unacknowledged for a later lifecycle boundary or a freshly attached Monitor. At other lifecycle boundaries, delivery remains separate and the next eligible Stop requests attachment. When work is queued and no lease is live, the bridge sends the attached peer one content-free frame, `parle: responsive delivery queued`; the Monitor notification starts a turn and the next lifecycle boundary injects the rows. A denied Monitor call or another Stop hook can leave the session visibly unarmed. In a live check, Claude Code's auto-mode permission classifier denied the Monitor call; the model must not work around a denial with Bash or a script, and the operator resolves it by allowing the Monitor tool (a permission rule) or switching permission mode, after which the next eligible Stop asks again. The supported remediation is otherwise the one Monitor call the hook asks for, then a Claude reload or restart, then a truthful upstream-blocked limitation. Claude exposes no host acknowledgement that a frame began a model turn, so live model-turn behavior remains an external validation concern. An unarmed period immediately after delivery is an expected transition, not by itself a bridge fault.

The Monitor task can end without delivering anything: `TaskStop`, a Claude restart, or a lost socket. The bridge sees only the detach; nothing was delivered or lost, and it keeps watching. The hook asks again once per eligible Stop, but after three detaches within an hour the bridge suspends idle wake, the Stop hook announces the suspension once (claimed, then committed only after the hook wrote its output), and `parle_status` reports `idle wake suspended: the wake attachment keeps closing` until the next prompt resets it; the suspension deliberately trades repeated re-attach noise for no idle wake at all until that next human prompt.

Operator note: since 0.9.71 Parle no longer appears as background work to Claude Code's goal check-ins, and `monitor_ws` tasks are outside the memory-pressure reaper that used to kill the Bash watcher, so the `CLAUDE_CODE_DISABLE_BG_SHELL_PRESSURE_REAP` opt-out is no longer needed for Parle. Sessions started before the plugin update still run the previous Stop hook, which asks for the removed `parle-watch.sh`; restart Claude Code to load the new hook (#194).
...
Config sources resolve in strict precedence: process environment, then `<cwd>/.env`. There is no project `.parle/credentials` file; a leftover one is inert. The MCP process loads once at server start. `PARLE_VERSION` is adapter-owned: persisted values are ignored with a warning and only process environment can override the default. The plugin never writes these files. A token rotated on disk after MCP launch requires a Claude Code restart. The Monitor attachment reads no configuration or credentials; the hook hands it the bridge's own address.
...
Live `parle_switch_profile` is disabled while the hook bridge owns responsive delivery. Restart Claude Code with the target `PARLE_PROFILE` so the MCP session, wake stream, queued delivery, Monitor attachment, and hook binding change together.
...
The Stop hook's attachment instruction carries only the bridge's loopback
address, never a plugin path, so a re-arm request can no longer name a stale
plugin cache (#194). A session started before a plugin update keeps running the
previous hook until Claude Code restarts; restart rather than re-invoking the
skill from a cache directory. Prune stale plugin cache versions after confirming
the current plugin is loaded.
