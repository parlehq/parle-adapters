...
The bridge scope is deliberately left unset so it derives from the process cwd, keeping the MCP socket and the hook that leases from it aligned per project. Host session binding is first-wins: a second Claude session sharing a project cannot steal a bound bridge, so every hook passes `--bind` to recover after an MCP restart without any risk of theft.

Hooks run at `SessionStart` (bind plus known-address context), `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `Stop`. `Stop` is required: a watcher wake can produce a turn that calls no tool, and without a terminal boundary that queued delivery would strand until the next prompt. Because `.mcp.json` env is snapshotted at launch, adopting this release requires a Claude Code restart.

The standalone watcher remains wake-only. It polls `projection` on a dedicated, unaliased session and never reads the responsive cursor, so it cannot double-drain or double-acknowledge alongside the bridge. It exists solely to wake an idle turn until Claude supports native push or channel delivery.
...
Config sources resolve in strict precedence: process environment, then `<cwd>/.env`. There is no project `.parle/credentials` file; a leftover one is inert. The MCP process loads once at server start. The standalone watcher resolves again through the bundled Node shared resolver on every invocation, including each manual re-arm. `PARLE_VERSION` is adapter-owned: persisted values are ignored with a warning and only process environment can override the default. The plugin never writes these files. A token rotated on disk after MCP launch requires a Claude Code session restart, while the next watcher re-arm reads the current profile. The watcher launcher bootstraps one dedicated, short-lived, unaliased entered session per arm so shared-room projection reads carry both required credentials without claiming the primary host's named route. The primary MCP session credential never crosses its process boundary. The watcher token and dedicated session credential pass only through private child environment, never argv, stdout, logs, or temporary files. When that dedicated session proactively rolls, the launcher terminates only its private worker and respawns it with the successor credential without returning to Claude. The original public worker arguments are preserved. This may replay the original `since_seq`, which is safe because projection filtering is idempotent. Primary-runtime session-id following remains a separate worker concern. The launcher retires its current dedicated session once when the watch finally exits.
