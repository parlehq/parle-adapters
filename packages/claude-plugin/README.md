...
Claude Code's memory-pressure reaper ends background shell tasks, including the watcher, when macOS reports memory pressure. The task then shows `status: killed` with empty output; nothing was delivered or lost, and the bridge keeps watching. The hook re-arms once per eligible Stop, but after three such detaches within an hour the bridge suspends idle wake, the Stop hook announces the suspension once, and `parle_status` reports `idle wake suspended: host memory pressure` until the next prompt resets it. On a busy development machine, `CLAUDE_CODE_DISABLE_BG_SHELL_PRESSURE_REAP=1` in the operator's own shell (before launching Claude Code) keeps the watcher alive; the plugin never sets it, because that is the host's safety valve to trade away, not Parle's.
