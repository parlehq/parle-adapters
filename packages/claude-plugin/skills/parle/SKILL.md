...
A watcher task notification with `status: killed` and empty output (or output that is only `[killed]`) is the host's memory-pressure reap of the background shell. It is not a fault and not "stopped by the host". Re-arm once in that same turn with the current-skill launcher, with no commentary before or after, and end the turn with no text. Exit 2 still means investigate. If the Stop hook reports idle wake suspended, do not re-arm and do not comment; delivery resumes at the next prompt. `parle_status` renders that state as `Delivery      watching (idle wake suspended: host memory pressure)`.
