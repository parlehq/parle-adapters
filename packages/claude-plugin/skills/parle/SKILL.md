...
3. Arming is part of connecting by default, but the Monitor call (next section) is made only when the Stop hook supplies the URL. When connect reports `idle_wake_unarmed`, do not render its intermediate `compactText` or explain the transition. Write only `Connected to <room>; attaching responsive delivery.`, end the turn, and make the Monitor call when the Stop hook supplies it. Once the Monitor task is running, call `parle_status` once and render its canonical `compactText` as the only connection card. Stand by without delivery only when the user explicitly asks. Do not infer delivery health from Monitor task creation, MCP connectivity, or remembered state.
...
1. The only trigger for calling Monitor is a Stop-hook instruction that supplies the URL: `Parle: call Monitor({ ws: { url: "..." }, persistent: true, description: "Parle responsive delivery" }) once.` When it arrives, arm by calling `Monitor({ ws: { url }, persistent: true, description: "Parle responsive delivery" })` with exactly that URL. The hook is the only place the URL appears: never construct or guess it, never take it from `parle_connect`, `parle_status`, or an earlier session. If no instruction has arrived yet, end the turn; the next eligible Stop asks once.
...
4. Never start a second Monitor while one is running. `idle_wake_unarmed` from `parle_connect` or `parle_status` is a report, not a trigger: those surfaces never carry the URL, so end the turn and wait for the hook's instruction. Never call `parle_status` repeatedly to wait for it.

If the host denies the Monitor call (a permission mode or classifier can), do not work around the denial with Bash, a script, or another tool. Tell the operator that responsive delivery needs the Monitor tool allowed (a permission rule) or a different permission mode, then end the turn; the next eligible Stop asks again.
