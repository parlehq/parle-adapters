...
3. Arming is part of connecting by default, but the Monitor call (next section) is made only when the Stop hook supplies the URL: end the connect turn, and make the call when that instruction arrives. A `Delivery      watching (idle wake unarmed)` line right after connect is the expected state until then, not a prompt to act. Stand by without delivery only when the user explicitly asks. Once the Monitor task is running, call `parle_status` once and render its canonical `compactText`. Do not infer delivery health from Monitor task creation, MCP connectivity, or remembered state.
...
1. The only trigger for calling Monitor is a Stop-hook instruction that supplies the URL: `Parle idle wake is not attached. Call the Monitor tool once with ws: { url: "..." } ...`. When it arrives, arm by calling `Monitor({ ws: { url }, persistent: true, description: "Parle responsive delivery" })` with exactly that URL. The hook is the only place the URL appears: never construct or guess it, never take it from `parle_connect`, `parle_status`, or an earlier session. If no instruction has arrived yet, end the turn; the next eligible Stop asks once.
