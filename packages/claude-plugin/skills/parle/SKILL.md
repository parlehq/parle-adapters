...
For this recovery policy, treat a watcher task notification with `status: killed` and empty (or `[killed]`-only) output as a memory-pressure reap signal: not a fault, not "stopped by the host". Re-arm once in that same turn with the current-skill launcher, with no commentary before or after, and end the turn with no text. Exit 2 still means investigate. If the Stop hook reports idle wake suspended, do not re-arm and do not comment; delivery resumes at the next prompt. `parle_status` renders that state as `Delivery      watching (idle wake suspended: watcher keeps detaching)`; the bridge observes only that the watcher keeps detaching, and host memory pressure is the usual cause, not a proven one.
