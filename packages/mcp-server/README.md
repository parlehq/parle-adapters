...
`PARLE_SESSION_ALIAS` is a requested alias only. Bootstrap, recovery, and host restart do not inspect, create, or claim it. `parle_session_alias` is the explicit assume action. If the alias is absent, it can proceed only when this local host already has human-owner custody, an exact `PARLE_AGENT_ID` in its environment or project `.env`, and the account and agent configuration resolve to the same API origin. The adapter uses the account client's fixed owned-agent creation capability only in that case. It never starts sign-in, mints credentials, or persists new credentials; otherwise the assume fails closed and asks for owner sign-in.
...
When `PARLE_RESPONSIVE_DELIVERY=hook-bridge`, the stdio process disables unread-count polling and starts the host-neutral hook delivery bridge. The bridge queues responsive rows in memory and exposes only server-framed delivery content over an owner-only Unix socket. A supported host binds the bridge to one exact host session, leases pending rows through lifecycle hooks, injects them through valid hook output, and commits before Parle acknowledgement. The socket never carries credentials. Session and alias delivery use independent server cursors. Every queued row retains the read's exact cursor scope and, for alias work, immutable alias identity and generation; deferred acknowledgement reuses that fence and never substitutes a successor alias context. A pending alias receipt therefore defers a generation-changing handoff rather than being acknowledged under new authority. `PARLE_HOOK_BRIDGE_SCOPE` selects a shared discovery scope when the MCP process and lifecycle hooks use different working directories. A host manifest with no idle-wake arm action sets `PARLE_HOST_IDLE_WAKE=none`; `parle_status` and `parle_connect` then report `responsiveDelivery.idleWake: "unavailable"` and never ask the model to arm a watcher.
