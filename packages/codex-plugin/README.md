...
2. The bridge resolves the parent's executable itself (`/proc/<pid>/exe` on Linux; `ps` then `lsof` on macOS) and accepts only an absolute path to a regular executable owned by the current user or by root (a system install such as `npm -g`), writable by neither group nor world, whose `--version` reports `codex-cli` 0.149.0 or newer; it refuses any other owner (`wrong-uid`), a loose mode (`unsafe-executable`), a changed parent, or a `--remote` topology. It never searches `PATH` and never runs a shell. Under the npm wrapper the native binary (`.../@openai/codex/vendor/<triple>/codex/codex`) is both the MCP server's parent and the hook's nearest Codex ancestor; the `node` shim above it is never consulted.
