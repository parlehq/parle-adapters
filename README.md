...
After shared client or MCP server changes, run `pnpm refresh:mcp-artifacts` before the reproducibility check. Commit the refreshed native Pi and Command Code bundles and three MCP wrapper artifacts, then apply the package version and changelog policy in `AGENTS.md`.
