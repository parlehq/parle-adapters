...
Codex runs hook commands through the user login shell with the session working directory. Ambient interpreter resolution is therefore not a safe plugin contract. The bridge publishes its exact `process.execPath` as owner-only runtime state beside the Unix socket. A stable fail-open launcher executes the JavaScript handler through that runtime handle and returns valid no-op JSON when responsive delivery is unavailable. The full decision, duplicate-delivery semantics, Windows boundary, and validation contract live in [Codex Hook Runtime](codex-hook-runtime.md).

Idle-time initiation uses Codex's own queue (0.149+): the bridge, correlated to the owning `codex` process and bound to the exact thread that both the hook and the MCP request metadata name, runs that process's `codex queue --thread <uuid> --message <fixed trigger>` when pending work first appears. The trigger carries no peer content; the queued turn fires UserPromptSubmit and the hook injects the content. The executable is the verified direct parent (absolute path, same uid, `codex-cli` >= 0.149.0), never a `PATH` lookup or a shell. Where that verification fails the adapter reports idle wake unavailable with a reason and messages wait for the next prompt; it still does not invent polling, cron, transcript editing, terminal automation, or a second Codex process.
